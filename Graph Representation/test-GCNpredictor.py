...
df_train = pd.read_csv('reddb_reaction_train.csv')
...
train_loader = DataLoader(dataset=list(zip(train_smiles, train_graph, train_y)), batch_size=128, collate_fn=collate_molgraphs,drop_last=True)
...
for batch_id, batch_data in enumerate(test_loader):
    ...
    pred = model(bg, bg.ndata['h'])
    all_pred.append(pred.data.cpu().numpy())
    

res=model(bg,bg.ndata['h']).detach().numpy()
